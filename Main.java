...
public class Main {
    ...
    private static void correctnessSize(int width){
        if (width<3){
            System.out.println("Данные не корректны, введенное значение меньше 3, построение изображения невозможно");
            System.exit(0);
        }

        if (width % 2 == 0){
            System.out.println("Данные не корректны, введнное является четным числом, построение изображения невозможно");
            System.exit(0);
        }
    }
